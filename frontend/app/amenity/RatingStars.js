const FilledStar = ({onClick, className}) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24" strokeWidth="1.5" stroke="white" className={`w-6 h-6 ${className}`} onClick={onClick}>
...
const EmptyStar = ({onClick, className}) => (
...
const RatingStars = ({ rating, category, setRating }) => {
    ...
    if (halfStar) {
        starElements.push(<HalfStar key={`half-star`} />);
    }
    ...
    for (let i = 0; i < emptyStars; i++) {
        starElements.push(
            <EmptyStar 
                key={`empty-star-${i}`} 
                onClick={() => handleStarClick(fullStars + halfStar + i + 1)} 
                className={setRating ? 'cursor-pointer' : ''}
            />
        );
    }
